      ...
      <div className="flex items-center text-base justify-between gap-4 mb-4">
        <div className="relative flex-auto">
          <Input
            ...
            name="filter-machine-serial-number"
          />
              ...
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header) => {
                  return (
                    <TableHead
                      key={header.id}
                      className="p-0 pl-6 text-gray-700 font-semibold"
                    >
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow key={row.id} className="h-12 text-gray-500">
                  {row.getVisibleCells().map((cell) => (
                    <TableCell
                      key={cell.id}
                      className="pl-6 last:justify-items-end last:pr-6"
                    >
                      {flexRender(
